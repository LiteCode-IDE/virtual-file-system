...
export const removeTabAsync = createAsyncThunk('removeTabAsync', async (_, { getState }) => {
    const state = getState();
    const normalized = state.structure.normalized;
    return normalized;
});
export const setActiveTabAsync = createAsyncThunk('setActiveTabAsync', async (id, { getState }) => {
    const state = getState();
    const normalized = state.structure.normalized;
...
export const tabsSlice = createSlice({
    ...
    reducers: {
        ...
        closeTab: (state, action) => {
            ...
            state.selectionStack = state.selectionStack.filter((id) => id !== action.payload);
            if (state.selected === action.payload) {
                const newSelectedStack = state.selectionStack.filter((id) => id !== action.payload);
    ...
    extraReducers: (builder) => {
        builder
            ...
            state.open = state.open.filter((tab) => normalized.files.allIds.find((id) => id === tab.id) !== undefined);
            state.selectionStack = state.selectionStack.filter((selected) => normalized.files.allIds.find((id) => id === selected) !== undefined);
            if (!state.open.find(({ id }) => id === state.selected)) {
                state.selected =
                    state.selectionStack[state.selectionStack.length - 1];
                state.selectionStack = state.selectionStack.slice(0, state.selectionStack.length - 1);
            }
        })
...
export const selectedTab = (state) => state.tabs.selected;
export const activeTabs = createSelector((state) => state.structure.normalized, (state) => state.tabs.open, (normalized, openTabs) => {
