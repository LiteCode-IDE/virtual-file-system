...
const validateFile = (preValidate, value, existingItems, validExtensions, item) => {
    // const regexp = new RegExp(/^(.*?)(\.[^.]*)?$/);
    const regex = /^([^\\]*)\.(\w+)$/;
    const regexFileName = /^[a-zA-Z0-9](?:[a-zA-Z0-9 ._-]*[a-zA-Z0-9])?\.[a-zA-Z0-9_-]+$/;
...
const validateFolder = (value, existingItems) => {
...
const validate = (preValidate, item, value, existingItems, validExtensions) => {
    if (item.type === "file") {
        return validateFile(preValidate, value, existingItems, validExtensions, item);
    }
    else if (item.type === "folder") {
